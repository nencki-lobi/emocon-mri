%-----------------------------------------------------------------------
% Job saved on 27-Oct-2020 17:16:08 by cfg_util (rev $Rev: 7345 $)
% spm SPM - SPM12 (7487)
% cfg_basicio BasicIO - Unknown
%-----------------------------------------------------------------------
matlabbatch{1}.spm.util.voi.spmmat = '<UNDEFINED>';
matlabbatch{1}.spm.util.voi.adjust = 1;
matlabbatch{1}.spm.util.voi.session = 1;
matlabbatch{1}.spm.util.voi.name = 'rpSTS';
matlabbatch{1}.spm.util.voi.roi{1}.sphere.centre = '<UNDEFINED>';
matlabbatch{1}.spm.util.voi.roi{1}.sphere.radius = '<UNDEFINED>';
matlabbatch{1}.spm.util.voi.roi{1}.sphere.move.fixed = 1;
matlabbatch{1}.spm.util.voi.roi{2}.mask.image = '<UNDEFINED>';
matlabbatch{1}.spm.util.voi.roi{2}.mask.threshold = 0.5;
matlabbatch{1}.spm.util.voi.expression = 'i1 & i2';
matlabbatch{2}.spm.stats.ppi.spmmat = '<UNDEFINED>';
matlabbatch{2}.spm.stats.ppi.type.ppi.voi(1) = cfg_dep('Volume of Interest:  VOI mat File', substruct('.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','voimat'));
matlabbatch{2}.spm.stats.ppi.type.ppi.u = '<UNDEFINED>';
matlabbatch{2}.spm.stats.ppi.name = '<UNDEFINED>';
matlabbatch{2}.spm.stats.ppi.disp = 1;
