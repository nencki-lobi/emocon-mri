...
ed_plot <- df_ca %>%
    ...
    #          dotsize = 0.4, alpha = 0.6, nrow = 1, ncolumn = 5, font.label = 1) %>%
    ...
    #ggboxplot(x = "group", y = "value", facet.by = "name", add = "jitter", width = 0.7,
    #          add.params = list(size = .8, alpha = .35),
    #          color = "group", palette = c("#D55E00", "#0072B2"),
    #          nrow = 1, legend = "top", legend.title = "")%>%
    #ggpar(yticks.by = 2) %>%
    #+ rremove("x.text") + rremove("x.ticks") + rremove("xlab") + theme(strip.text.x = element_text(size = 8))

    ggbarplot(x = "group", y = "value", facet.by = "name", add = c("mean_ci", "jitter"),
              add.params = list(size = .8, alpha = .35),
              color = "group", palette = c("#D55E00", "#0072B2"),
              nrow = 1, legend = "top", legend.title = "")%>%
    ggpar(yticks.by = 2) %>%
...
ggexport(
    ed_plot,
    filename = "figures/evaluation_of_demonstrator_barplot.tiff",
